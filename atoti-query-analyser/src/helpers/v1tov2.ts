...
import {Retrieval, Plan, QueryFilter} from './plan-types';
...
const parseNewRetrieval = (_line, match): RetrievalV1 => ({
...
type ParsingResult = {
  ...
  retrievals: {[key: string]: RetrievalV1},
  current: null | RetrievalV1,
  parents: any
}
const parseV1 = (input, tickCallback): Promise<ParsingResult> => {
  return new Promise(resolve => {
    const accumulator: ParsingResult = {
      info: {
        contextValues: {},
        globalTimings: {}
      },
...
const createDependencyList = (v1Structure: ParsingResult) => {
...
const createFilterMap = (v1Structure: ParsingResult): QueryFilter[] => {
  ...
  return iterators.reduce(
    ...
    (acc, [filter, idx]) => {
      acc[idx] = {
        id: idx, 
        description: filter === GLOBAL_FILTER ? v1Structure.rootFilter : filter
      };
      return acc;
    },
    []);
...
const findFilter = (filters: QueryFilter[], needle: string) => {
  if (needle === GLOBAL_FILTER || needle === undefined) {
    return 0;
  }
  const filter = filters.find(f => f.description === needle);
  return filter.id;
};

const createRetrievalMap = (v1Structure: ParsingResult, filters): Retrieval[] => {
...
const convertToV2 = (v1Structure: any): Plan[] => {
  ...
  if (retrievals[0].timingInfo === undefined) {
